...
def _filter_span_tags(parent_tag, block_tag):
    if parent_tag is None or isinstance(parent_tag, NavigableString) or parent_tag.name != 'span':
        return None

    parent_classes = parent_tag.get('class')
    if parent_classes is None:
        parent_classes = []

    block_classes = block_tag.get('class')
    if block_classes is None:
        block_classes = []

    parent_classes.extend(block_classes)

    s = [(parent_tag, list(set(parent_classes)))]
            ...
            for child_tag in children:
                if isinstance(child_tag, NavigableString):
                    ...
                    if child_tag.name == 'br':
                        if len(block_tag.contents) > 0 and is_valid_string(block_tag.text, r'\s+'):
                            parent_tag.insert_after(block_tag)
                            parent_tag = block_tag

                        # Điều kiện để tận dụng lại block tag chưa sử dụng
                        if len(block_tag.contents) > 0:
                            block_tag = create_html_tag('div', attrs=block_tag.attrs)
                    else:  # Xem các thẻ còn lại là span
                        # Lấy thẻ span con ra để khi thẻ cha bị xóa thì không bị xóa theo
                        span_tag = child_tag.extract()
                        ...
                        if len(span_tag.contents) > 0:
                            s.append((span_tag, list(set(span_classes))))
                        else:
                            span_tag.decompose()
        ...
        if children_size > 0:
            ...
            while i < children_size:
                ...
                if isinstance(child_tag, NavigableString):
                    ...
                    temp_tag = _filter_span_tags(parent_tag=child_tag, block_tag=temp_tag)
                else:
...
def main():
    ...
    <div class="main">
     <div class="a">
     Khoa</br>
     <div class="p">Demo<div class="e">Khoa</div></div>
     </div>
     <br/>
     <div class="b">
      <span class="x">
    ...
    <div class="a">
        <div class="b">
            <span class="c"><span>Khoa</span><br/>Khoa3</span>
        </div>
    </div>
    '''
    s = get_soup(d, True)
