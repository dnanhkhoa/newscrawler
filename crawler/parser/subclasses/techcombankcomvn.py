...
class TechcombankComVnParser(SubBaseParser):
    def __init__(self):
        ...
        def get_next_url_func(html, url):
            ul_tag = html.find('ul', class_='paging')
            if ul_tag is None:
                return None
            a_tag = ul_tag.find(lambda x: x.name == 'a' and 'Trang sau' in x.text)
        ...
        def get_post_urls_func(html):
            ...
            # Child posts
            ul_tag = html.find('ul', class_='entries1-type1')
            if ul_tag is None:
                return urls
            ...
            for post in posts:
                ...
                time_tag = post.find('span', class_='publishtime')
                if a_tag is not None and time_tag is not None:
                    time = time_tag.text.strip()
                    urls.append((a_tag.get('href'), datetime.strptime(time, '%m/%d/%Y %I:%M:%S %p')))
