...
class TechcombankComVnParser(SubBaseParser):
    def __init__(self):
        ...
        self._vars['get_title_tag_func'] = lambda x: x.find('h1')
        ...
        # Gán bằng con trỏ hàm hoặc biểu thức lambda
        self._vars['get_time_tag_func'] = lambda x: x.find('div', class_='publishtime')

        # Định dạng chuỗi thời gian và trả về đối tượng datetime
        # Gán bằng con trỏ hàm hoặc biểu thức lambda
        def get_datetime_func(string):
            return datetime.strptime(string.strip(), '%m/%d/%Y %I:%M:%S %p')
        ...
        # Gán bằng con trỏ hàm hoặc biểu thức lambda
        self._vars['get_main_content_tag_func'] = lambda x: x.find('div', class_='column_middle')
    ...
    def _pre_process(self, html):
        tag = html.find('ul', class_='breadcrumb')
        if tag is not None:
            tag.decompose()
        ...
        tag = html.find('div', class_='publishtime')
        if tag is not None:
            tag.decompose()

        tag = html.find('div', class_='slideshow')
        if tag is not None:
            tag.decompose()

        tags = html.find_all('style')
        for tag in tags:
            tag.decompose()

        tags = html.find_all('div', class_='clear')
        for tag in tags:
            tag.decompose()

        tag = html.find('div', class_='new-relation2')
        if tag is not None:
            tag.decompose()

        tag = html.find('div', class_='content_footer')
        if tag is not None:
            tag.decompose()
