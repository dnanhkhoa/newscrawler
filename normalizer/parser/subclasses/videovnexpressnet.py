...
class VideoVnexpressNetParser(SubBaseParser):
    def __init__(self):
        ...
        self._source_page = 'Video VNExpress'
        ...
        # Gán bằng con trỏ hàm hoặc biểu thức lambda
        self._vars['get_summary_tag_func'] = lambda x: x.find('div', class_='lead_detail_video')
    ...
    def _get_og_image(self, html):
        a_tag = html.find('a', attrs={'class': 'fb_share', 'onclick': True})
        if a_tag is None:
            return None

        matcher = regex.search(r"VideoVNE\.shareFB\('[^']+', '([^']+)'\)", a_tag.get('onclick'), regex.IGNORECASE)
        if matcher is None:
            return None
    ...
    def _get_publish_date(self, html):
        ...
        matcher = regex.search(r'{"articlePublishDate": "([^"]+)"}', tag.text, regex.IGNORECASE)
        if matcher is None:
            return None

        matcher = regex.search(r'(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})', matcher.group(1), regex.IGNORECASE)
        if matcher is None:
            return None

        time_string = '%s/%s/%s %s:%s:%s' % (
    ...
    def _get_meta_keywords(self, html):
        ...
        matcher = regex.search(r'{"articleTags": "([^"]+)"}', tag.text, regex.IGNORECASE)
        if matcher is None:
            return None
    ...
    def _parse(self, url, html, timeout=15):
        ...
        return super()._parse(matcher.group(1), html, timeout)
