...
class NhanDanComVnParser(SubBaseParser):
    def __init__(self):
        ...
        def get_time_tag_func(html):
            div_tag = html.find('div', class_='icon_date_top')
            return None if div_tag is None else div_tag.find('div', class_='pull-left')
        ...
        def get_datetime_func(string):
            parts = string.split(', ')
            return datetime.strptime('%s %s' % (parts[-1], parts[1]), '%H:%M:%S %d/%m/%Y')
        ...
        # Gán bằng con trỏ hàm hoặc biểu thức lambda
        self._vars['get_main_content_tag_func'] = lambda x: x.find('div', class_='media', style='padding-right:10px;')
    ...
    def _pre_process(self, html):
        div_tag = html.find('div', class_='ndtitle')
